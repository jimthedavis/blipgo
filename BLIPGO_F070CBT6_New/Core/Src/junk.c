...
uint16_t meter_sequence_number = 0;
uint16_t total_downloaded_records_meter = 0;
...
#if INDIA
const unsigned char apn[40]="airtelgprs.com";
#elif USA
const unsigned char apn[40]="data641003";
#endif


uint8_t rx_string_esp[300];
uint8_t tx_string[400];
...
uint8_t twin_rp[100];
uint8_t read_pb[100];
const uint8_t protocol_version[5] = {"1.0"};
...
uint8_t stored_settings[300];
uint8_t stored_parameters[100];
...
uint8_t read_device_twin(void);
void send_text_to_uart1(uint8_t *str);
void wait_ms(uint16_t t);
uint8_t at_cmd_send_esp(char *at_command, char *success_response,char *error_response, uint16_t delay, uint8_t high_prioity_task);
uint8_t at_cmd_send(char *at_command, char *success_response,char *error_response, uint16_t delay, uint8_t receive_sms_flag);
uint8_t ble_initialize_gatt_client(char* str);
void ble_get_paired_device(void);
void ble_scan_available_device(void);
uint8_t ble_is_available_device_paired(void);
void beep(uint8_t duration);
uint8_t send_softap_response(char* str, uint16_t len);
void send_text_to_usb(char *str);
uint8_t ping_service(void);
uint8_t json_get_value(char* parameter, uint8_t parameter_length, uint8_t occurance, char* variable, uint8_t variable_size);
uint8_t configuration_service(void);
uint8_t ping_service(void);
uint8_t get_root_ca_certificate(void);
uint8_t get_client_certificate(void);
uint8_t get_client_key(void);
uint8_t get_server_clock(void);
uint8_t connect_mqtt_server(void);
uint8_t update_device_twin(void);
uint8_t read_settings_from_flash(uint32_t sect_address);
...
uint8_t read_runtime_parameters_from_flash(uint32_t sect_address);
void store_runtime_parameters_to_flash(uint32_t sect_address);
uint8_t publish_meter_readings(void);
...
uint8_t get_gsm_signal_strength(void);
uint8_t get_module_firmware_version(void);
uint8_t update_module_firmware(void);
...
uint8_t get_ntp_clock(void);
uint8_t activate_pdp(void);
uint8_t deactivate_pdp(void);
...
uint8_t __ROM_Page_Erase_(uint32_t page_address, uint32_t number_of_pages)
{
...
uint8_t ftp_download(char* ftp_server, char* ftp_user, char* ftp_pswd, uint16_t ftp_port, char* ftp_filename)
{
    ...
    if(at_cmd_send((char*)tx_string,"OK\r\n","ERROR",5,0))
    {
        ...
        if(at_cmd_send("AT+QIACT?\r\n\0","QIACT:","ERROR",5,0))
        {
            ...
            sprintf((char*)tx_string,
                    "AT+QFTPOPEN=\"%s\",%u\r\n",
                    ftp_server,
                    (unsigned int)ftp_port); /////OPEN FTP CONNECTION
            ...
            if(at_cmd_send((char*)tx_string,"QFTPOPEN:","ERROR",95,0))
            {
                ...
                {
                at_cmd_send("AT+QFTPLIST=\".\"\r\n","QFTPLIST:","ERROR",60,0);//+QFTPLIST: 0
                HAL_Delay(1000);
                ...
                HAL_Delay(300);
                    flush_tx_string();
                    sprintf((char*)tx_string, "AT+QFTPGET=\"%s\",\"%s\"\r\n", ftp_filename, ftp_filename);////////DOWNLOAD FILE
...
//                        HAL_Delay(100);
                        ...
                        ///+QFTPCLOSE: 0,0
                        at_cmd_send("AT+QFTPCLOSE\r\n","QFTPCLOSE: 0","ERROR",120,0);////////CLOSE FTP CONNECTION
                        HAL_Delay(100);
...
uint8_t esp_enable_softap_mode(void)
{
...
uint8_t update_device_firmware(void)
{
...
if(activate_pdp())
{
    ...
    at_cmd_send("at+qhttpurl?\r\n\0","OK\r\n","ERROR",5,0);
        ...
        sprintf((char*)tx_string2, "%sHost: shark.carematix.com\r\n", tx_string);
        sprintf((char*)tx_string, "%sContent-Type: application/json\r\n", tx_string2);
        sprintf((char*)tx_string2, "%sContent-Length: 160\r\n", tx_string);
        ...
        */
        sprintf((char*)tx_string2, "%s{\"model\":\"%s\",\"sim\":\"%s\",\"imei\":\"%s\",\"imsi\":\"%s\",\"cv\":%s,\"fv\":\"%s\",\"pv\":\"%s\",\"mv\":\"%s\"}\r\n",
...
//        sprintf((char*)at_command, "at+qhttpget=80,%u\r\n\0", strlen((char*)tx_string));
        sprintf((char*)at_command,
                "at+qhttppost=%u,80,80\r\n",
                strlen((char*)tx_string2));

    if(at_cmd_send((char*)at_command,"CONNECT\r\n","ERROR",70,0))
    {
        HAL_Delay(10);

        if(at_cmd_send((char*)tx_string2, "QHTTPPOST: 0", "ERROR", 70, 0))//+QHTTPGET: 0,200,80
        {
        ...
        if(at_cmd_send("AT+QHTTPREAD=80\r\n\0","QHTTPREAD: 0\r\n","ERROR",70,0))///+QHTTPREAD: 0
        {
            ...
            if(configuration_version[0]!=temp_data[0] || configuration_version[1]!=temp_data[1] || configuration_version[2]!=temp_data[2])
            {
                ...
                {
                send_text_to_usb("Client Certificate: ");
                send_text_to_usb((char*)temp_data);
                send_text_to_usb("\r\n");
...
if(activate_pdp())
{
    ...
    if(at_cmd_send("at+qhttpget=80\r\n\0","QHTTPGET: 0","ERROR",70,0))
    {
        ...
        if(at_cmd_send("AT+QHTTPREAD=80\r\n\0","QHTTPREAD: 0\r\n","ERROR",70,0))///+QHTTPREAD: 0
        {
            ...
            {
            send_text_to_usb("Server Host Name: ");
...
if(activate_pdp())
{
    ...
    if(strstr((char*)gsm_data, "QNTP: 0"))
    {
        ...
        sprintf((char*)ntp_format, "%s", temp_data);
...
//    send_text_to_usb("\r\n\0");
        ...
        send_text_to_usb("\r\nEpoch Time: \0");
        sprintf((char*)temp_data, "%ld", (long) server_clock);
        send_text_to_usb((char*)temp_data);
        send_text_to_usb("\r\n\0");
...
if(at_cmd_send("AT+CSQ\r\n\0", "OK\r\n", "ERROR\r\n", 5, 0))
    ...
    for(; start<15; start++)
    {
        ...
        if(gsm_data[start+2]=='Q')
        {
            ...
            flush_tx_string();
            sprintf((char*)tx_string, "\r\nSignal Strength : %u\r\n", (unsigned int)gsm_signal_strength);
            send_text_to_usb((char*)tx_string);
...
while(mem_write_address>mem_read_address)
{
    ...
    sprintf((char*)serial_number.dataValue, "%s", device_id);
    serial_number.dataLength=strlen((char*)serial_number.dataValue);

    sprintf((char*)string2, "%02X%c%02X%s%c",
    ...
    sprintf((char*)timestamp.dataValue, "%02X%02X%02X%02X", bytes[0], bytes[1], bytes[2], bytes[3]);
    timestamp.dataLength=strlen((char*)timestamp.dataValue);

    sprintf((char*)string3, "%02X%c%02X%s%c",
    ...
    {
        sprintf((char*)model_number.dataValue, "262");
    }
    else
    {
        sprintf((char*)model_number.dataValue, "%s", read_meter_model);/// if other meter
    }

    model_number.dataLength=strlen((char*)model_number.dataValue);

    sprintf((char*)string4, "%02X%c%02X%s%c",
    ...
    sprintf((char*)meter_id.dataValue, "%s", read_meter_serial_no);
    meter_id.dataLength=strlen((char*)meter_id.dataValue);

    sprintf((char*)string5, "%02X%c%02X%s%c",
    ...
    sprintf((char*)string6, "%02X%c%02X%s%c",
    ...
    sprintf((char*)signal_strength.dataValue, "%02X", gsm_signal_strength);

    signal_strength.dataLength=strlen((char*)signal_strength.dataValue);

    sprintf((char*)string7, "%02X%c%02X%s%c",
    ...
    sprintf((char*)string1, "{'data':'S3%02X%02X", packet_length[1], packet_length[0]);////// length 6 bytes

    //// send string1+string2
    flush_tx_string();
    sprintf((char*)tx_string, "%s%s%s%s%s%s%sZ'}\r\n%c", string1, string2, string3, string4, string5, string6, string7, 0x1A);
...
if(at_cmd_send((char*)tx_string,"QMTSUB: 0,1,0,0\r\n","ERROR",20,0))/// +QMTSUB: 0,1,0,0
{
    ...
    flush_tx_string();
    sprintf((char*)tx_string, "%c", 0x1A);/////// CTRL+Z
...
{
flush_tx_string();
sprintf((char*)tx_string, "at+qmtpub=0,1,1,0,\"%s\"\r\n", twin_rp);/////PUBLISH TO TWIN_RP TOPIC
if(at_cmd_send((char*)tx_string,">","ERROR",20,0))/// >
{
    flush_tx_string();
...
{
    sprintf((char*)server_clock_format, "#clock%u*", (unsigned int)server_clock);
//    send_text_to_uart2(server_clock_format);// Debug TTL
    ...
    sprintf((char*)server_clock_format, "#msrl%s*", meter_serial_no);
//    send_text_to_uart2(server_clock_format);// Debug TTL
...
if(at_cmd_send("AT+QGMR\r\n\0","OK\r\n","ERROR",5,0))
{
    ...
    sprintf((char*)module_firmware_version, "%s", temp_data);
...
{
    sprintf((char*)configuration_version, "%s", (char*)hold_cv_value);
    store_settings_to_flash(DEVICE_SETTINGS_ADDRESS);
    store_settings_to_flash(DEVICE_SETTINGS_BACKUP_ADDRESS);
    read_settings_from_flash(DEVICE_SETTINGS_ADDRESS);
}
...
send_text_to_uart2((uint8_t *)"ATE0\r\n");
send_text_to_uart1((uint8_t *)"ATE0\r\n");
HAL_Delay(300);
...
send_text_to_uart2((uint8_t *)"AT+QURCCFG=\"urcport\",\"uart1\"\r\n");
HAL_Delay(300);
...
sprintf((char*)ble_ssid_name, "BlipGo+%s", device_id);
...
//    at_cmd_send_esp("AT+CWMODE=1\r\n\0", "OK\r\n", "ERROR", 10, 0);
    ...
    if(ble_initialize_gatt_client((char*)ble_ssid_name))
    {
        ...
        sprintf((char*)meter_model, "%s", temp_data);
        }
        send_text_to_usb((char*)"\r\nModel: \0");
        ...
        sprintf((char*)meter_serial_no, "%s", temp_data);
        }
        send_text_to_usb((char*)"\r\nMeter SN: ");
        ...
        uint8_t pairing_success_flag=0;
...
//    HAL_Delay(500);
    ...
    flush_tx_string();
    sprintf((char*)tx_string, "%c%c",4, 1);
    send_text_to_uart1(tx_string);
    HAL_Delay(500);
    ...
    sprintf((char*)tx_string, "%c%c%c%c%c%c%c",racp_opcode, racp_operator, racp_filter, meter_sequence_char[0], meter_sequence_char[1],meter_sequence_char2[0], meter_sequence_char2[1]);
...
//    HAL_Delay(100);
    ...
    //////// PRINT METER RECORDS
    uint8_t write_server_clock_flag=1;
    ...
    for(uint8_t cc = 0; cc < 100; cc++)
    {
        ...
        flush_tx_string();
        sprintf((char*)tx_string, "Record No. %u: ", (unsigned int)total_downloaded_records_meter);
        send_text_to_usb((char*)tx_string);
